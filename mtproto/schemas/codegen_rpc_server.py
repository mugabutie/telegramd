...
classTypesTexts = 'type %sServiceImpl struct {\n}\n\n' %(to_proto_first_name(service_name))
...
for restype in funcsList:
  ...
  for data in v:
    ...
    if (to_proto_first_name(service_name) != to_proto_first_name(name)):
      continue

    #resType2 = TypesDict[''.restype]
    resType = FuncsDict[restype]
    if (resType.find('Vector') >= 0):
      classTypesTexts += '// func (s *%sServiceImpl)%s(ctx context.Context,  request *mtproto.TL%s) (*mtproto.%s, error) {\n' %(to_proto_first_name(service_name), to_proto_go_name(name), to_proto_go_name(name), to_proto_go_name(resType))
      classTypesTexts += '//   glog.Info("Process: %v", request)\n'
      classTypesTexts += '//   return nil, nil\n'
      classTypesTexts += '// }\n\n'
    else:
      classTypesTexts += 'func (s *%sServiceImpl)%s(ctx context.Context,  request *mtproto.TL%s) (*mtproto.%s, error) {\n' %(to_proto_first_name(service_name), to_proto_go_name(name), to_proto_go_name(name), to_proto_go_name(resType))
      classTypesTexts += '  glog.Info("Process: %v", request)\n'
